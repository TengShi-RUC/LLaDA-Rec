...
def ensure_finite_(x: torch.Tensor,
...
def activation_checkpoint_function(cfg: ModelConfig):
    preserve_rng_state = ((cfg.attention_dropout == 0.0)
...
class LayerNormBase(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if elementwise_affine or (elementwise_affine is None
                                  and self.config.layer_norm_with_affine):
    ...
    def _cast_if_autocast_enabled(
        ...
        if tensor.device.type == "cuda" and torch.is_autocast_enabled():
            return tensor.to(dtype=dtype if dtype is not None else torch.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.low_precision:
            ...
            with torch.autocast(enabled=False, device_type=module_device.type):
                return F.layer_norm(downcast_x,
...
class RotaryEmbedding(nn.Module):
    ...
    def get_rotary_embedding(
        ...
        if ((pos_sin := self.__cache.get("rope_pos_sin")) is not None
        ...
        with torch.autocast(device.type, enabled=False):
            ...
            inv_freq = 1.0 / (self.rope_theta**(torch.arange(
                0, dim, 2, device=device, dtype=torch.float) / dim))
            seq = torch.arange(seq_len, device=device, dtype=torch.float)
            freqs = einsum("i , j -> i j", seq, inv_freq)
            positions = torch.cat((freqs, freqs), dim=-1)
            pos_sin, pos_cos = positions.sin()[
                None, None, :, :], positions.cos()[None, None, :, :]
        self.__cache["rope_pos_sin"] = pos_sin
        self.__cache["rope_pos_cos"] = pos_cos
    ...
    def apply_rotary_pos_emb(self, pos_sin: torch.Tensor,
    ...
    def forward(self, q: torch.Tensor,
        ...
        with torch.autocast(q.device.type, enabled=False):
            query_len, key_len = q_.shape[-2], k_.shape[
                -2]  # could be different if layer_past not None
...
def get_causal_attention_bias(cache: BufferCache, seq_len: int,
                              device: torch.device) -> torch.Tensor:
    if (causal_bias := cache.get("causal_attention_bias")
...
def alibi_attention_bias(seq_len: int, config: ModelConfig,
                         device: torch.device) -> torch.FloatTensor:
    alibi_bias = torch.arange(1 - seq_len, 1, dtype=torch.float,
                              device=device).view(1, 1, 1, seq_len)

    # shape: (1, 1, seq_len, seq_len)
    alibi_bias = alibi_bias - torch.arange(
        1 - seq_len, 1, dtype=torch.float, device=device).view(
            1, 1, seq_len, 1)
    ...
    return alibi_bias * (1.0 /
                         (2**m.view(1, config.n_heads, 1, 1)))  # type: ignore
...
class LLaDABlock(nn.Module):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.attn_out = nn.Linear(config.d_model,
                                  config.d_model,
    ...
    def _scaled_dot_product_attention(
        ...
        if self.flash_attn_func is not None and attn_mask is None:
            r = self.flash_attn_func(q.transpose(1, 2),
                                     k.transpose(1, 2),
                                     v.transpose(1, 2),
                                     dropout_p=dropout_p,
                                     causal=False)
            ...
            if num_q_heads != num_kv_heads:
                ...
                k = k.repeat_interleave(num_q_heads // num_kv_heads,
                                        dim=1,
                                        output_size=num_q_heads)
                v = v.repeat_interleave(num_q_heads // num_kv_heads,
                                        dim=1,
                                        output_size=num_q_heads)

            # Modify: MDM set causal to False, and with no attn_mask.
            ### NOTE: this is different from original LLaMA code.
            # print((attn_mask!=0).sum())
    ...
    def attention(
        ...
        # shape: (B, nh, T, hs)
        q = q.view(B, T, self.config.n_heads,
        ...
        query_len, key_len = q.shape[-2], k.shape[
            -2]  # could be different if layer_past not None
        ...
        att = self._scaled_dot_product_attention(
            ...
            k,
            v,
            attn_mask=attention_bias,  ### ATTENTION BIAS
            dropout_p=0.0
            if not self.training else self.config.attention_dropout,
            is_causal=False,
        )
    ...
    @classmethod
    def build(cls, layer_id: int, config: ModelConfig,
              cache: BufferCache) -> LLaDABlock:
...
class LLaDASequentialBlock(LLaDABlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.att_proj = nn.Linear(config.d_model,
                                  sum(self.fused_dims),
                                  bias=config.include_bias
                                  | config.include_qkv_bias,
                                  device=config.init_device)
        # Feed-forward input projection.
        self.ff_proj = nn.Linear(config.d_model,
                                 self.hidden_size,
                                 bias=config.include_bias,
                                 device=config.init_device)
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            q, k, v = self.att_proj(
                self._activation_checkpoint_fn(self.attn_norm,
                                               x)).split(self.fused_dims,
                                                         dim=-1)
        else:
            q, k, v = self.att_proj(self.attn_norm(x)).split(self.fused_dims,
                                                             dim=-1)
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        self.q_proj = nn.Linear(config.d_model,
                                q_proj_out_dim,
                                bias=config.include_bias
                                | config.include_qkv_bias,
                                device=config.init_device)
        self.k_proj = nn.Linear(config.d_model,
                                k_proj_out_dim,
                                bias=config.include_bias
                                | config.include_qkv_bias,
                                device=config.init_device)
        self.v_proj = nn.Linear(config.d_model,
                                v_proj_out_dim,
                                bias=config.include_bias
                                | config.include_qkv_bias,
                                device=config.init_device)
        ...
        self.ff_proj = nn.Linear(config.d_model,
                                 ...
                                 device=config.init_device)
        # new add
        self.up_proj = nn.Linear(config.d_model,
                                 self.hidden_size,
                                 bias=config.include_bias,
                                 device=config.init_device)
...
class LLaDABlockGroup(nn.ModuleList):
    ...
    def __init__(self,
        ...
        self.activation_checkpointing_strategy: Optional[
            ActivationCheckpointingStrategy] = None
        self._activation_checkpoint_fn = activation_checkpoint_function(
            self.config)
    ...
    def forward(
        ...
        for block_idx, block in enumerate(self):
            ...
            if ((self.activation_checkpointing_strategy
                 == ActivationCheckpointingStrategy.whole_layer)
                    or (self.activation_checkpointing_strategy
...
class LLaDAModel(nn.Module):
    ...
    def __init__(self, config: ModelConfig, init_params: bool = True):
        ...
        if not (0 < self.config.block_group_size <= self.config.n_layers
                and self.config.n_layers % self.config.block_group_size == 0):
            raise Exception("n layers must be divisible by block group size")

        torch.backends.cuda.enable_flash_sdp(True)
        torch.backends.cuda.enable_mem_efficient_sdp(
            False)  # this is super slow so make sure torch won't use it

        self.transformer = nn.ModuleDict(
            dict(
                wte=nn.Embedding(config.embedding_size or config.vocab_size,
                                 config.d_model,
                                 device=config.init_device),
        ...
        blocks = [
            LLaDABlock.build(i, config, self.__cache)
            for i in range(config.n_layers)
        ]
        if self.config.block_group_size > 1:
            block_groups = [
                LLaDABlockGroup(config, i,
                                blocks[i:i + config.block_group_size])
        ...
        if self.config.alibi:
            get_causal_attention_bias(self.__cache, config.max_sequence_length,
                                      _non_meta_init_device(config))
            self.get_alibi_attention_bias(config.max_sequence_length,
                                          _non_meta_init_device(config))
    ...
    def reset_parameters(self):
        ...
        init_weights(
            ...
            if self.config.scale_logits else 1.0,
            type_of_module=ModuleType.emb,
        )
        if hasattr(self.transformer, "wpe"):
            init_weights(self.config,
                         self.transformer.wpe,
                         type_of_module=ModuleType.emb)  # type: ignore
        ...
        if hasattr(self.transformer, "ff_out"):
            init_weights(self.config,
                         ...
                         type_of_module=ModuleType.final_out)  # type: ignore
    ...
    def get_alibi_attention_bias(self, seq_len: int,
                                 device: torch.device) -> torch.Tensor:
        if (alibi_bias := self.__cache.get("alibi_attention_bias")
    ...
    ###
    def get_bidirectional_attention_bias(self, seq_len: int,
                                         device: torch.device) -> torch.Tensor:
        if (bidirectional_bias :=
                self.__cache.get("bidirectional_attention_bias")
            ...
            if bidirectional_bias.device != device:
                ...
                self.__cache[
                    "bidirectional_attention_bias"] = bidirectional_bias
            return bidirectional_bias
        with torch.autocast(device.type, enabled=False):
            bidirectional_bias = torch.zeros((1, 1, seq_len, seq_len),
                                             device=device,
                                             dtype=torch.float)
    ...
    def forward(
        ...
        # TODO: check 在推荐中不一定需要?

        assert (past_key_values is None
                and not use_cache), "The kvcache is not suppotred for MDM."
        ...
        x = self.transformer.wte(
            input_ids
        ) if input_embeddings is None else input_embeddings  # type: ignore
        ...
        if (attention_bias is not None or attention_mask is not None
            ...
            if attention_bias is None and self.config.alibi:
                attention_bias = get_causal_attention_bias(
                    ...
                    x.device) + self.get_alibi_attention_bias(
            ...
            if attention_mask is not None:
                ## get bidirectional attention bias, padding positions are -inf
                attention_bias = attention_bias + attention_mask
                # Might get -infs after adding attention mask, since dtype.min + dtype.min = -inf.
                # `F.scaled_dot_product_attention()` doesn't handle -inf like you'd expect, instead
                # it can produce NaNs.
                ensure_finite_(attention_bias,
                               check_neg_inf=True,
                               check_pos_inf=False)

        attn_key_values: Optional[List[Tuple[
            torch.Tensor, torch.Tensor]]] = [] if use_cache else None
        # print("Debug: attention_bias", attention_bias)
        # print(attention_bias.shape if attention_bias is not None else None)
        # none zero
        # print((attention_bias != 0).sum() if attention_bias is not None else None)
        # decoder layers
        all_hidden_states = []
        ...
        if self.config.block_group_size == 1:
            ...
            for group_idx, block_group in enumerate(
                    self.transformer.block_groups):
                if output_hidden_states:
                    # add hidden states
                    all_hidden_states.append(x)

                layers_past = (None if past_key_values is None else
                               past_key_values[group_idx *
                                               self.config.block_group_size:
                                               (group_idx + 1) *
                                               self.config.block_group_size])
                x, cache = block_group(x,
                                       attention_bias=attention_bias,
        ...
        return LLaDAOutput(
            ...
            if output_hidden_states else None)  # type: ignore[arg-type]
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[
            Cache] = None,  # This is a hack mitigation of an issue in transformers `4.39.x`
    ...
    def prepare_inputs_for_generation(
        ...
        model_inputs["use_cache"] = kwargs.pop("use_cache",
