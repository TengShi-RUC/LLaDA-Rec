...
class AmazonReviews2023(AbstractDataset):
    ...
    def _process_meta(self, output_path: str):
        ...
        if process_mode == 'sentence':
            ...
            item2meta = self._extract_meta_sentence_features(
                meta_dataset=meta_dataset)
        else:
            raise NotImplementedError(
                f'Metadata processing mode "{process_mode}" not implemented.')
            ...
            if os.path.isdir(raw_data_dir):
                ...
                with self.accelerator.main_process_first(
                    ...
                    datasets = load_dataset(
                        ...
                        data_files={
                            split:
                            f"{HF_url}/{self.config['kcore']}/{self.config['split']}_w_his/{self.category}.{split}.csv"
                            for split in ['train', 'valid', 'test']
                        },
